...
def runFEAforDCB(model_lib, Design_space, length, radius, angle,ifmaterial,materialdensity,materialtable,
              materialsection,meshpart,meshsize,anslysislib,Staticnlgeom,Bendingdirection,StaticCPUnum,Staticsaveresult,
              Bucklingdirection,Bucklingmode,defectcoefficient,saveresult,NonLinearCPUnum,Eigenreq,RefBC,Inertiamass,freqCPUnum):

    """
        Main Function: Integrates modeling, material setting, meshing, and various analyses.

        This function handles the entire workflow, calling the relevant sub-modules as needed.
    """

    radius = float(radius)
    angle = float(angle)
    length = float(length)
    Design_space = float(Design_space)

    model = mdb.models['Model-1']

    def Modeling(model_lib, Design_space, length, radius, angle):
        if model_lib == 'CTLT':
            l1 = length
            r1 = radius
            a1 = angle
            ...
            model.sketches['profile'].ArcByCenterEnds(
                ...
                point1=(-r1 * sin(a1 * pi / 180.0), -r1 * cos(a1 * pi / 180.0)),
                point2=(-r1 * sin(a1 * pi / 180.0) - r11 * sin(a1 * pi / 180.0), 0.0))
            model.Part(dimensionality=THREE_D, name='CTLT', type=
                DEFORMABLE_BODY)
            model.parts['CTLT'].BaseShellExtrude(depth=l1, sketch=
        ...
        if model_lib == 'TRAC':
            ...
            model.Part(dimensionality=THREE_D, name='TRAC', type=
                DEFORMABLE_BODY)
            model.parts['TRAC'].BaseShellExtrude(depth=l2, sketch=
                model.sketches['profile'])
            del model.sketches['profile']
        ...
        if model_lib == 'CBOOM':
            ...
            model.Part(dimensionality=THREE_D, name='CBOOM', type=
                DEFORMABLE_BODY)
            model.parts['CBOOM'].BaseShellExtrude(depth=l3, sketch=
                model.sketches['profile'])
            del model.sketches['profile']
    ...
    def MaterialSetting(Design_space, length, radius, angle,materialdensity, materialtable, materialsection):
        for partName, part in model.parts.items():
            if partName == 'CTLT':
                ...
                ##Material##
                model.parts[partName3].Set(faces=
                    model.parts[partName3].faces.findAt(
                ...
                model.parts[partName3].SectionAssignment(
                    offset=0.0,
                    offsetField='',
                    offsetType=MIDDLE_SURFACE,
                    region=model.parts[partName3].sets['cboom_section'],
                    sectionName='Section-single',
                    thicknessAssignment=FROM_SECTION)

                model.parts[partName3].MaterialOrientation(additionalRotationType=
                                                           ROTATION_NONE, axis=AXIS_2, fieldName='',
                                                           localCsys=None, orientationType=
                                                           GLOBAL, region=
                                                           model.parts[partName3].sets[
                                                               'cboom_section'])

            else:
                print('Part {} does not match any specified names'.format(partName))
    ...
    def Meshing( Design_space, length, radius, angle,meshsize):
        for partName, part in model.parts.items():
            if partName == 'CTLT':
                l1 = length
                r1 = radius
        ...
        for instanceName, instance in model.rootAssembly.instances.items():
            print("Instance Name: {}".format(instanceName))
            elementTypes = {}
            for element in instance.elements:
                elementType = element.type
                if elementType in elementTypes:
                    elementTypes[elementType] += 1
                else:
                    elementTypes[elementType] = 1

            for elemType, count in elementTypes.items():
                print("Element Type: {}, Count: {}".format(elemType, count))
    ...
    def NonLinearBuckling_analysis(Bucklingdirection, Bucklingmode, defectcoefficient, saveresult, NonLinearCPUnum):
        ...
        mdb.models['Model-LinearBuckling'].DisplacementBC(name='BC-1', createStepName='Initial',
                                                          region=model.rootAssembly.sets['Ref1'], u1=SET, u2=SET,
                                                          u3=SET, ur1=SET,
                                                          ur2=SET, ur3=SET,
                                                          amplitude=UNSET, distributionType=UNIFORM, fieldName='',
                                                          localCsys=None)
        mdb.models['Model-LinearBuckling'].DisplacementBC(name='BC-2', createStepName='Step-LinearBuckle',
                                                          region=model.rootAssembly.sets['Ref2'], u1=UNSET, u2=SET,
                                                          u3=-1.0, ur1=UNSET,
                                                          ur2=UNSET, ur3=UNSET,
                                                          amplitude=UNSET, distributionType=UNIFORM, fieldName='',
                                                          localCsys=None)
        ...
        mdb.models['Model-LinearBuckling'].keywordBlock.insert(64, """
                       *node file
                       u""")
        ##Job##
        ...
        if Bucklingdirection == 'X':
            mdb.models['Model-NonLinearBuckling'].DisplacementBC(name='BC-2', createStepName='Step-NonLinearBuckle',
                                                                 region=model.rootAssembly.sets['Ref2'], u1=0.0,
                                                                 u2=UNSET, u3=UNSET,
                                                                 ur1=-1.0, ur2=0.0, ur3=0.0,
        ...
        if Bucklingdirection == 'Y':
            mdb.models['Model-NonLinearBuckling'].DisplacementBC(name='BC-2', createStepName='Step-NonLinearBuckle',
                                                                 region=model.rootAssembly.sets['Ref2'], u1=UNSET,
                                                                 u2=0.0, u3=UNSET,
                                                                 ur1=0.0, ur2=-1.0, ur3=0.0,
                                                                 amplitude=UNSET, distributionType=UNIFORM,
                                                                 fieldName='',
        ...
        new_line = "*imperfection,file=ctltLinear,Step=1\n{},{}\n".format(Bucklingmode, defectcoefficient)
    ...
    Modeling(model_lib, Design_space, length, radius, angle)

    if ifmaterial == True:

       MaterialSetting(Design_space, length, radius, angle,materialdensity, materialtable, materialsection)

    if meshpart == True:

       Meshing(Design_space, length, radius, angle,meshsize)
